...
interface TaskContextType {
    ...
    setToast: React.Dispatch<React.SetStateAction<{ message: string; type: "success" | "danger" } | null>>;
}
...
export function TaskContextProvider({ children }: TaskContextProviderProps) {
    ...
    const createTask = useCallback(async (data: CreateTaskData): Promise<Task> => {
        try {
            const taskData = { ...data, completed: false, createdAt: new Date().toISOString(), finishedAt: null };
    ...
    const updateTaskCompleted = useCallback(async (id: number, completed: boolean): Promise<Task> => {
        try {
            const task = tasks.find(t => t.id === id);
            if (!task) throw new Error("Task not found");

            const updatedTask = { ...task, completed, finishedAt: completed ? new Date().toISOString() : null };
            const response = await apiPrivate.put<Task>(`${API_URL}/${id}`, updatedTask);
